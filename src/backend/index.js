...
app.post('/employees/employer', async (req, res) => {
  try {
    ...
    const employees = await employeeService.findEmployeesByEmployer(employerName);
    res.json(employees);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.put('/employees/:number', async (req, res) => {
  try {
    ...
    const updated = await employeeService.updateEmployee(req.params.number, req.body);
    res.json(updated);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.post('/query/:collection', async (req, res) => {
  try {
    const result = await database.executeRawQuery(req.params.collection, req.body);
    res.json(result);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
